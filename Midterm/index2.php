...
<html>
	...
	<form action = "index2.php" method = "POST" name = "Quiz">
	<?php
	...
	*/
	...
	foreach ($questions as $q => $text) {
		...
		print "<br>$text[text]<br>"; //Question Text 'text'
		foreach ($text['choices'] as $question => $abc) {
			print "<br>$question: "; //Question answer letter
			print "<input type = 'radio' name = '$q' value='$question' />"; //Prints radio button per option
			print htmlspecialchars(" $abc"); //Question answer text
			print "<br>"; //Used for proper spacing
			if(isset($_POST[$q])) { //This checks each radio button has been set.
				if($text['correct'] == $_POST[$q]) { //This works when the right radio button is clicked
					//print "***CORRECT!!! $text[correct] -- $_POST[$q]"; //Prints when the answer is correct (test)
					$check[] = ["qnum" => $q, "answer" => 'Correct']; //Adds Question # and answer to associative array
				} 
				else { 
					//print "***INCORRECT!"; //Prints when incorrect (test)
					$check[] = ["qnum" => $q, "answer" => 'Incorrect']; //Adds Question # and answer to associative array
				}
			} 
			else { 
				$check[] = ["qnum" => $q, "answer" => 'Pick a Choice']; //Checks to insure all answer have been answered
			} 
		}
		print "<br><br><hr><br>"; //Line Break
		//print "<br>Correct Answer: $text[correct]<br><br>"; //Answer 'correct'
	} 
	$next = null; //Used so answers are not printed in duplicates
	print "<br><hr><br>"; //Line Break
	print "<table border='2' cellspacing='2'>"; //Creating table for answers
	print "<th>Your answers: </th>";
	foreach($check as $qn => $ans ) {
		//print "<br><br><br><br>$qn<br><br>$ans[qnum]<br>$ans[answer]"; //Used to check variables (test)
		if($ans['qnum'] !== $next) {
			print "<tr><td>" . "  $ans[qnum]" . "</td>" . "<td>" . "$ans[answer]  " . "</td></tr>"; //Prints out answers in table form
		}			
		$next = $ans['qnum']; //Used for duplicates
	}
	print "</table>";
	?>
